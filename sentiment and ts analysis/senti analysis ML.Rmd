...
prepare_ml <- function (dtm, original_data) {
  ...
  ml_list <- list("train_data" = train_data, "test_data" = test_data, 
                  "train_label" = cbind(train_pos_label, train_neg_label),
                  "test_label" = cbind(test_pos_label, test_neg_label),
                  "test_df" = test_df)
...
create_cm <- function (test_df) {
  ...
  hybrid <- cut(test_df$hybrid, 
                   ...
                   labels = c("N","O","P"))
  confusionMatrix(table(hybrid, test_df$senti_label), positive = "P", mode = "everything") 
}
```
